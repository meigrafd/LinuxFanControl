/*
 * Linux Fan Control (LFC) - Shared Memory Telemetry
...
namespace lfc { namespace shm {
    ...
    inline void destroy(Mapping& m) {
        if (m.hdr) { ::munmap(static_cast<void*>(m.hdr), m.size); m.hdr=nullptr; m.frames=nullptr; }
    ...
    inline bool createOrOpen(Mapping& m, const char* shmName, uint32_t capacity, bool create) {
        ...
        if (create) {
            ...
            if (need < sizeof(RingHeader)) { destroy(m); return false; }
        }
        ...
        } else if (m.hdr->magic != kMagic || m.hdr->version != kVersion) {
            destroy(m); return false;
        }
        m.owner = create;
        return true;
    }
    ...
    inline bool writeFrame(Mapping& m, const char* id, double duty, double temp, uint64_t ts_ns) {
        ...
        if (id && *id) std::strncpy(f.id, id, sizeof(f.id)-1);
        f.duty = duty;
        f.temp = temp;
        return true;
    }
