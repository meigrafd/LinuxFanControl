...
void Detection::worker() {
    ...
    for (size_t i = 0; i < snap_.pwms.size() && !stop_.load(); ++i) {
        ...
        int baseline = 0;
        for (const auto& f : snap_.fans) baseline = std::max(baseline, Hwmon::readRpm(f).value_or(0));

        Hwmon::setManual(pwm);
        Hwmon::setPercent(pwm, 100);

        for (int t = 0; t < 20 && !stop_.load(); ++t) std::this_thread::sleep_for(std::chrono::milliseconds(100));

        int peak = 0;
        for (const auto& f : snap_.fans) peak = std::max(peak, Hwmon::readRpm(f).value_or(0));
