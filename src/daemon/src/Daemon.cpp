...
namespace lfc {
  ...
  void Daemon::bindCommands(CommandRegistry& reg) {
    reg.registerMethod("lfc.ping", "Health check", [](const RpcRequest&) -> RpcResult {
      return {true, "{\"ok\":true}"};
    });

    reg.registerMethod("lfc.cmds", "List available commands", [this](const RpcRequest&) -> RpcResult {
    ...
    reg.registerMethod("engine.enable", "Enable/disable automatic control; params: {\"on\":bool}", [this](const RpcRequest& req) -> RpcResult {
    ...
    reg.registerMethod("engine.status", "Basic engine status", [this](const RpcRequest&) -> RpcResult {
      std::ostringstream ss;
    ...
    reg.registerMethod("hwmon.snapshot", "Counts of discovered devices", [this](const RpcRequest&) -> RpcResult {
      std::ostringstream ss;
    ...
    reg.registerMethod("telemetry.json", "Return current SHM JSON blob", [this](const RpcRequest&) -> RpcResult {
      std::string s;
